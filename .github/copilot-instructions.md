...
| **SecurityIncident** | **Phantom incidents with empty `AlertIds`:** The SecurityIncident table contains hundreds of records synced from Defender XDR with `AlertIds = []` (empty array). These are auto-resolved incidents that **never appear in the Defender XDR portal queue** or the `ListIncidents` Graph API. In one validated environment, 276 of 315 "Closed" incidents had empty AlertIds — inflating closed counts by ~7x vs the portal's 42. `TimeGenerated > ago(7d)` further inflates by capturing old incidents with recent status updates. LLMs default to `TimeGenerated` and don't filter AlertIds, producing wildly incorrect incident counts | **For portal-consistent closed incident counts:** (1) Use `CreatedTime` (not `TimeGenerated`) for "incidents created in window" semantics, (2) Add `\| where array_length(AlertIds) > 0` to exclude phantom incidents. For open incidents (`Status in ("New", "Active")`), the `mv-expand AlertId = AlertIds` naturally drops empty-AlertId rows, so the filter is less critical but still recommended for explicit correctness |
