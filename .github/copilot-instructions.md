...
| **DeviceTvmSoftwareVulnerabilities / DeviceTvmSoftwareInventory / DeviceTvmSecureConfigurationAssessment / SecurityRecommendation** | **Advanced Hunting only** — Defender TVM tables do NOT exist in Sentinel Data Lake | Always use `RunAdvancedHuntingQuery`. Uses `Timestamp` not `TimeGenerated`. For vulnerability investigations, use the `exposure-investigation` skill |
